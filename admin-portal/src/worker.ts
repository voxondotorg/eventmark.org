...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (pathname === "/styles.css") return css(adminStyles());
  ...
  <div class="admin-layout">
    <aside class="sidebar">
      ...
      <nav class="sidebar-nav">
        <a href="/" class="nav-item ${activeNav === "dashboard" ? "active" : ""}"><span class="nav-code">OV</span> Dashboard</a>
        <a href="/users" class="nav-item ${activeNav === "users" ? "active" : ""}"><span class="nav-code">US</span> Users <span class="badge" id="verification-badge" style="display:none">0</span></a>
        <a href="/organizations" class="nav-item ${activeNav === "organizations" ? "active" : ""}"><span class="nav-code">OR</span> Organizations</a>
        <a href="/events" class="nav-item ${activeNav === "events" ? "active" : ""}"><span class="nav-code">EV</span> Events</a>
        <a href="/org-requests" class="nav-item ${activeNav === "org-requests" ? "active" : ""}"><span class="nav-code">RQ</span> Org Requests <span class="badge" id="pending-badge" style="display:none">0</span></a>
        <a href="/audit-logs" class="nav-item ${activeNav === "audit-logs" ? "active" : ""}"><span class="nav-code">LG</span> Audit Logs</a>
        <a href="/settings" class="nav-item ${activeNav === "settings" ? "active" : ""}"><span class="nav-code">ST</span> Settings</a>
      </nav>
      <div class="sidebar-footer">
        <a href="/cdn-cgi/access/logout" class="nav-item logout"><span class="nav-code">BY</span> Sign out</a>
      </div>
    </aside>
...
function renderDashboard(title: string, user: AuthContext): string {
  return renderBaseHtml(title, user, `
    <div class="page-header"><h2>Dashboard</h2><p>Overview of your EventMark instance</p></div>
    <div class="stats-grid">
      <div class="stat-card"><div class="stat-value" id="stat-users">-</div><div class="stat-label">Total Users</div></div>
      <div class="stat-card"><div class="stat-value" id="stat-orgs">-</div><div class="stat-label">Organizations</div></div>
      <div class="stat-card"><div class="stat-value" id="stat-events">-</div><div class="stat-label">Events</div></div>
      <div class="stat-card"><div class="stat-value" id="stat-pending">-</div><div class="stat-label">Pending Org Requests</div></div>
      <div class="stat-card"><div class="stat-value" id="stat-verification-pending">-</div><div class="stat-label">Verification Requests</div></div>
    </div>
    <div class="quick-actions">
      <h3>Quick Actions</h3>
      <div class="action-buttons">
        <a href="/org-requests" class="btn btn-primary">Review Org Requests</a>
        <a href="/users?verification=pending" class="btn btn-primary">Review Verifications</a>
        <a href="/users" class="btn btn-secondary">Manage Users</a>
        <a href="/events" class="btn btn-secondary">Moderate Events</a>
      </div>
    </div>
    <div class="recent-activity">
      <h3>Recent Activity</h3>
      <div id="activity-list" class="activity-list"><p>Loading...</p></div>
    </div>
  `, "dashboard");
}
...
function renderUsersPage(title: string, user: AuthContext): string {
  return renderBaseHtml(title, user, `
    <div class="page-header"><h2>Users</h2><p>Manage EventMark users</p></div>
    <div class="filters">
      <input type="text" id="user-search" placeholder="Search by email or name..." class="search-input">
...
function renderUserDetailPage(title: string, user: AuthContext, userId: string): string {
  return renderBaseHtml(title, user, `
    <div class="page-header"><a href="/users" class="back-link">← Back to Users</a><h2>User Details</h2></div>
...
function renderOrganizationsPage(title: string, user: AuthContext): string {
  return renderBaseHtml(title, user, `
    <div class="page-header"><h2>Organizations</h2><p>Manage approved organizations</p></div>
    <div class="table-container">
      <table class="data-table">
...
function renderOrganizationDetailPage(title: string, user: AuthContext, orgId: string): string {
  return renderBaseHtml(title, user, `
    <div class="page-header"><a href="/organizations" class="back-link">← Back to Organizations</a><h2>Organization Details</h2></div>
...
function renderEventsPage(title: string, user: AuthContext): string {
  return renderBaseHtml(title, user, `
    <div class="page-header"><h2>Events</h2><p>Moderate all events</p></div>
    <div class="filters">
...
function renderEventDetailPage(title: string, user: AuthContext, eventId: string): string {
  return renderBaseHtml(title, user, `
    <div class="page-header"><a href="/events" class="back-link">← Back to Events</a><h2>Event Details</h2></div>
...
function renderOrgRequestsPage(title: string, user: AuthContext): string {
  return renderBaseHtml(title, user, `
    <div class="page-header"><h2>Organization Requests</h2><p>Review and approve organization applications</p></div>
    <div class="filters">
...
function renderAuditLogsPage(title: string, user: AuthContext): string {
  return renderBaseHtml(title, user, `
    <div class="page-header"><h2>Audit Logs</h2><p>Track all admin actions</p></div>
    <div class="filters">
  ...
  return renderBaseHtml(title, user, `
    ...
    <form id="settings-form" class="settings-form">
      <div class="form-group">
        ...
        <textarea id="admin-emails" name="adminEmails" rows="3" placeholder="admin@example.com, super@example.com"></textarea>
        <p class="help-text">These emails will have admin access automatically</p>
      </div>
      <div class="form-group">
        <label for="notice-banner">Site Notice Banner</label>
        <textarea id="notice-banner" name="noticeBanner" rows="2" placeholder="Optional notice to display on all pages"></textarea>
      </div>
      <div class="form-group checkbox-group">
        <label><input type="checkbox" id="pause-org-requests" name="pauseOrgRequests"> Pause new organization requests</label>
      </div>
      <div class="form-group checkbox-group">
...
function adminStyles(): string {
  return `
:root { --bg-primary: #0d0d0d; --bg-secondary: #141414; --bg-tertiary: #1e1e1e; --bg-elevated: #252526; --text-primary: #f4f4f4; --text-secondary: #b0b0b0; --text-muted: #7d7d7d; --accent-primary: #ffffff; --accent-success: #d7ffd7; --accent-danger: #ff5d5d; --accent-warning: #f0d48b; --border-color: #2a2a2a; --radius: 8px; --font-main: 'Segoe UI', system-ui, -apple-system, sans-serif; --font-mono: 'JetBrains Mono', 'SF Mono', Menlo, Consolas, monospace; }
* { box-sizing: border-box; margin: 0; padding: 0; }
body { font-family: var(--font-main); background: var(--bg-primary); color: var(--text-primary); line-height: 1.5; }
body::before { content: ''; position: fixed; inset: 0; pointer-events: none; background-image: linear-gradient(rgba(255,255,255,0.02) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.02) 1px, transparent 1px); background-size: 24px 24px; opacity: 0.22; z-index: -1; }
.admin-layout { display: flex; min-height: 100vh; }
.sidebar { width: 260px; background: var(--bg-secondary); border-right: 1px solid var(--border-color); display: flex; flex-direction: column; position: sticky; top: 0; min-height: 100vh; }
.sidebar-header { padding: 1.5rem; border-bottom: 1px solid var(--border-color); }
.sidebar-kicker { color: var(--text-muted); font-family: var(--font-mono); font-size: 0.72rem; letter-spacing: 0.12em; text-transform: uppercase; margin-bottom: 0.4rem; }
.sidebar-header h1 { font-size: 1.25rem; margin-bottom: 0.25rem; font-family: var(--font-mono); }
.sidebar-subtitle { color: var(--text-secondary); font-size: 0.85rem; margin-bottom: 0.75rem; }
.user-info { font-size: 0.82rem; color: var(--text-muted); word-break: break-word; }
.sidebar-nav { flex: 1; padding: 1rem 0; }
.sidebar-footer { padding: 1rem 0; border-top: 1px solid var(--border-color); margin-top: auto; }
.nav-item { display: flex; align-items: center; gap: 0.75rem; padding: 0.85rem 1.5rem; color: var(--text-secondary); text-decoration: none; transition: all 0.2s; font-family: var(--font-mono); font-size: 0.8rem; }
.nav-item:hover { background: var(--bg-tertiary); color: var(--text-primary); }
.nav-item.active { background: var(--bg-tertiary); color: var(--accent-primary); border-right: 2px solid var(--accent-primary); }
.nav-code { display: inline-flex; align-items: center; justify-content: center; min-width: 2rem; height: 1.5rem; border: 1px solid var(--border-color); border-radius: 999px; color: var(--text-muted); font-size: 0.68rem; }
.nav-item.active .nav-code { color: var(--accent-primary); border-color: var(--accent-primary); }
.nav-item .badge { margin-left: auto; background: var(--accent-danger); color: white; padding: 0.125rem 0.5rem; border-radius: 9999px; font-size: 0.75rem; }
.nav-item.logout { color: var(--text-muted); }
.nav-item.logout:hover { color: var(--accent-danger); }
.main-content { flex: 1; padding: 2rem; overflow-y: auto; }
.page-header { margin-bottom: 2rem; }
.page-header h2 { font-size: 1.75rem; margin-bottom: 0.5rem; }
.page-header p { color: var(--text-secondary); }
.back-link { display: inline-block; margin-bottom: 1rem; color: var(--text-secondary); text-decoration: none; font-family: var(--font-mono); font-size: 0.8rem; }
.back-link:hover { color: var(--text-primary); }
.stats-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-bottom: 2rem; }
.stat-card { background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: var(--radius); padding: 1.5rem; }
.stat-value { font-size: 2rem; font-weight: 600; color: var(--accent-primary); margin-bottom: 0.5rem; font-family: var(--font-mono); }
.stat-label { color: var(--text-secondary); font-size: 0.8rem; font-family: var(--font-mono); }
.filters { display: flex; gap: 1rem; margin-bottom: 1.5rem; }
.search-input { flex: 1; max-width: 400px; padding: 0.65rem 1rem; background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: var(--radius); color: var(--text-primary); font-family: var(--font-mono); }
.filter-select { padding: 0.65rem 1rem; background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: var(--radius); color: var(--text-primary); font-family: var(--font-mono); }
...
.data-table th { text-align: left; padding: 0.75rem 1rem; background: var(--bg-tertiary); font-weight: 600; font-size: 0.78rem; color: var(--text-secondary); font-family: var(--font-mono); text-transform: uppercase; letter-spacing: 0.04em; }
.data-table td { padding: 0.75rem 1rem; border-top: 1px solid var(--border-color); }
.data-table tr:hover { background: var(--bg-tertiary); }
.btn { display: inline-flex; align-items: center; gap: 0.5rem; padding: 0.55rem 1rem; border-radius: 4px; font-weight: 500; text-decoration: none; border: 1px solid var(--border-color); cursor: pointer; font-size: 0.78rem; font-family: var(--font-mono); }
.btn-primary { background: var(--accent-primary); color: #111; border-color: var(--accent-primary); }
.btn-secondary { background: var(--bg-tertiary); color: var(--text-secondary); }
.btn-danger { background: var(--accent-danger); color: white; }
.btn-sm { padding: 0.25rem 0.75rem; font-size: 0.75rem; }
.btn:hover { background: var(--bg-elevated); color: var(--text-primary); }
.btn-primary:hover { background: #e8e8e8; color: #111; }
.form-group { margin-bottom: 1.5rem; }
.form-group label { display: block; margin-bottom: 0.5rem; font-weight: 500; font-family: var(--font-mono); font-size: 0.8rem; }
.form-group input, .form-group textarea, .form-group select { width: 100%; padding: 0.65rem 0.75rem; background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: var(--radius); color: var(--text-primary); font-family: var(--font-mono); }
.form-group input:focus, .form-group textarea:focus, .form-group select:focus { outline: none; border-color: var(--accent-primary); }
.help-text { margin-top: 0.25rem; font-size: 0.875rem; color: var(--text-muted); }
...
.activity-list { background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: var(--radius); padding: 1rem; }
...
.request-card { background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: var(--radius); padding: 1.5rem; }
.request-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 1rem; }
.request-title { font-size: 1.125rem; font-weight: 600; }
...
.status-badge.pending { background: var(--accent-warning); color: var(--bg-primary); }
.row-highlight { background: rgba(210, 153, 34, 0.12); }
tr.row-highlight td { border-top-color: var(--accent-warning); border-bottom-color: var(--accent-warning); }
.status-badge.approved { background: var(--accent-success); color: #111; }
.status-badge.rejected { background: var(--accent-danger); color: white; }
.status-badge.draft, .status-badge.archived, .status-badge.published, .status-badge.organizer, .status-badge.admin, .status-badge.user, .status-badge.visitor { background: var(--bg-elevated); color: var(--text-primary); }
.request-actions { display: flex; gap: 0.5rem; margin-top: 1rem; }
.detail-grid { display: grid; grid-template-columns: 1.4fr 1fr; gap: 1rem; }
.detail-stack { display: grid; gap: 1rem; }
.meta-list { display: grid; gap: 0.65rem; }
.meta-list p { color: var(--text-secondary); }
.meta-list strong { color: var(--text-primary); }
.mini-list { display: grid; gap: 0.5rem; margin-top: 0.75rem; }
.mini-row { border-top: 1px solid var(--border-color); padding-top: 0.5rem; }
.error-page { text-align: center; padding: 4rem; }
.error-page h1 { font-size: 4rem; margin-bottom: 1rem; color: var(--accent-danger); }
...
@media (max-width: 900px) {
  ...
  .nav-item.active { border-right: 1px solid var(--accent-primary); }
...
@media (max-width: 640px) {
  ...
  .stat-card,
  .table-container,
  .activity-list { border-radius: 6px; }
  .request-card,
...
async function loadUsers() {
  ...
  try {
    ...
    tbody.innerHTML = data.items.map(u => {
      ...
      const verificationPill = pendingVerification ? ' <span class="status-badge pending">Verification pending</span>' : '';
      return \`<tr\${rowClass}><td>\${escapeHtml(u.email)}\${verificationPill}</td><td>\${escapeHtml(u.name || '-')}</td><td>\${u.roles.map(r => \`<span class="status-badge \${r}">\${r}</span>\`).join(' ')}</td><td>\${u.verified ? '<span class="status-badge approved" title="Verified">✓ Verified</span>' : '<span class="status-badge" style="background:var(--bg-elevated);color:var(--text-muted)">Unverified</span>'}</td><td>\${u.organizationIds.length}</td><td>\${formatDate(u.createdAt)}</td><td><a href="/users/\${u.id}" class="btn btn-sm btn-secondary">View</a></td></tr>\`;
    }).join('');
  ...
  try {
    ...
    const verificationActions = verificationPending ? \`
        <div class="request-actions" style="margin-top:1rem">
          <button id="btn-approve-verification" class="btn btn-primary" data-userid="\${escapeHtml(user.id)}">Approve verification</button>
          <button id="btn-reject-verification" class="btn btn-secondary" data-userid="\${escapeHtml(user.id)}">Reject request</button>
        </div>\` : '';
    container.innerHTML = \`
      <div class="request-card\${verificationPending ? ' row-highlight' : ''}">
        <div class="request-header">
          ...
          \${verificationPending ? '<span class="status-badge pending">Verification pending</span>' : isVerified
            ? '<span class="status-badge approved">✓ Verified</span>'
            : '<span class="status-badge" style="background:var(--bg-elevated);color:var(--text-muted)">Unverified</span>'}
        </div>
        <p><strong>ID:</strong> \${escapeHtml(user.id)}</p>
        <p><strong>Roles:</strong> \${user.roles.map(r => \`<span class="status-badge \${r}">\${r}</span>\`).join(' ')}</p>
        ...
        <div class="request-actions" style="margin-top:1.5rem">
          <button id="btn-toggle-verify" class="btn \${isVerified ? 'btn-secondary' : 'btn-primary'}" data-verified="\${isVerified}" data-userid="\${escapeHtml(user.id)}">
...
async function loadOrganizations() {
  ...
  try {
    ...
    tbody.innerHTML = data.items.map(o => \`<tr><td>\${escapeHtml(o.name)}</td><td><a href="\${escapeHtml(o.website)}" target="_blank">\${escapeHtml(o.website)}</a></td><td><span class="status-badge \${o.vettingStatus.toLowerCase()}">\${o.vettingStatus}</span></td><td>\${o.memberCount || 0}</td><td>\${formatDate(o.createdAt)}</td><td><a href="/organizations/\${o.id}" class="btn btn-sm btn-secondary">View</a></td></tr>\`).join('');
...
async function loadOrganizationDetail(orgId) {
  ...
  try {
    ...
    const eventList = events.length
      ? events.map(event => \`<div class="mini-row"><strong>\${escapeHtml(event.title)}</strong><p>\${formatDate(event.startsAt)} · <span class="status-badge \${event.status}">\${escapeHtml(event.status)}</span></p></div>\`).join('')
      ...
      <div class="detail-grid">
        <section class="request-card detail-stack">
          ...
          <div class="meta-list">
            ...
            <p><strong>Status:</strong> <span class="status-badge \${org.vettingStatus.toLowerCase()}">\${escapeHtml(org.vettingStatus)}</span></p>
...
async function loadEvents() {
  ...
  try {
    ...
    tbody.innerHTML = data.items.map(e => \`<tr><td>\${escapeHtml(e.title)}</td><td>\${escapeHtml(e.organizationName || '-')}</td><td><span class="status-badge \${e.status}">\${e.status}</span></td><td>\${formatDate(e.startsAt)}</td><td><a href="/events/\${e.id}" class="btn btn-sm btn-secondary">View</a></td></tr>\`).join('');
      ...
      <div class="detail-grid">
        <section class="request-card detail-stack">
          ...
          <div class="meta-list">
            ...
            <p><strong>Status:</strong> <span class="status-badge \${event.status}">\${escapeHtml(event.status)}</span></p>
...
async function loadAuditLogs() {
  ...
  try {
    ...
    tbody.innerHTML = data.items.map(log => \`<tr><td>\${formatDate(log.createdAt)}</td><td><span class="status-badge">\${log.action.replace(/_/g, ' ')}</span></td><td>\${escapeHtml(log.actor)}</td><td>\${escapeHtml(log.targetName)}</td><td>\${log.details ? '<button onclick="alert(this.dataset.details)" data-details="\${escapeHtml(log.details)}" class="btn btn-sm btn-secondary">View</button>' : '-'}</td></tr>\`).join('');
  } catch (e) { tbody.innerHTML = \`<tr><td colspan="5">Error: \${e.message}</td></tr>\`; }
}
